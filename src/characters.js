import Albedo from './img/Albedo.png'
import Alhaitham from './img/Alhaitham.png'
import Aloy from './img/Aloy.png'
import Amber from './img/Amber.png'
import Arataki_Itto from './img/Arataki_Itto.png'
import Arlecchino from './img/Arlecchino.png'
import Baizhu from './img/Baizhu.png'
import Barbara from './img/Barbara.png'
import Beidou from './img/Beidou.png'
import Bennet from './img/Bennett.png'
import Candace from './img/Candace.png'
import Charlotte from './img/Charlotte.png'
import Chevreuse from './img/Chevreuse.png'
import Chiori from './img/Chiori.png'
import Chongyun from './img/Chongyun.png'
import Clorinde from './img/Clorinde.png'
import Collei from './img/Collei.png'
import Cyno from './img/Cyno.png'
import Dehya from './img/Dehya.png'
import Diluc from './img/Diluc.png'
import Diona from './img/Diona.png'
import Dori from './img/Dori.png'
import Eula from './img/Eula.png'
import Faruzan from './img/Faruzan.png'
import Fischl from './img/Fischl.png'
import Freminet from './img/Freminet.png'
import Furina from './img/Furina.png'
import Gaming from './img/Gaming.png'
import Ganyu from './img/Ganyu.png'
import Gorou from './img/Gorou.png'
import Hu_Tao from './img/Hu_Tao.png'
import Jean from './img/Jean.png'
import Kaedehara_Kazuha from './img/Kaedehara_Kazuha.png'
import Kaeya from './img/Kaeya.png'
import Kamisato_Ayaka from './img/Kamisato_Ayaka.png'
import Kamisato_Ayato from './img/Kamisato_Ayato.png'
import Kaveh from './img/Kaveh.png'
import Keqing from './img/Keqing.png'
import Kirara from './img/Kirara.png'
import Klee from './img/Klee.png'
import Kujou_Sara from './img/Kujou_Sara.png'
import Kuki_Shinobu from './img/Kuki_Shinobu.png'
import Layla from './img/Layla.png'
import Lisa from './img/Lisa.png'
import Lynette from './img/Lynette.png'
import Lyney from './img/Lyney.png'
import Mika from './img/Mika.png'
import Mona from './img/Mona.png'
import Nahida from './img/Nahida.png'
import Navia from './img/Navia.png'
import Neuvillette from './img/Neuvillette.png'
import Nilou from './img/Nilou.png'
import Ningguang from './img/Ningguang.png'
import Noelle from './img/Noelle.png'
import Qiqi from './img/Qiqi.png'
import Raiden_Shogun from './img/Raiden_Shogun.png'
import Razor from './img/Razor.png'
import Rosaria from './img/Rosaria.png'
import Sangonomiya_Kokomi from './img/Sangonomiya_Kokomi.png'
import Sayu from './img/Sayu.png'
import Sethos from './img/Sethos.png'
import Shenhe from './img/Shenhe.png'
import Shikanoin_Heizou from './img/Shikanoin_Heizou.png'
import Sigewinne from './img/Sigewinne.png'
import Sucrose from './img/Sucrose.png'
import Tartaglia from './img/Tartaglia.png'
import Thoma from './img/Thoma.png'
import Tighnari from './img/Tighnari.png'
import traveler_Anemo from './img/traveler(Anemo).png'
import traveler_Dendro from './img/traveler(Dendro).png'
import traveler_Electro from './img/traveler(Electro).png'
import traveler_Geo from './img/traveler(Geo).png'
import traveler_Hydro from './img/traveler(Hydro).png'
import Venti from './img/Venti.png'
import Wanderer from './img/Wanderer.png'
import Wriothesley from './img/Wriothesley.png'
import Xiangling from './img/Xiangling.png'
import Xianyun from './img/Xianyun.png'
import Xiao from './img/Xiao.png'
import Xingqiu from './img/Xingqiu.png'
import Xinyan from './img/Xinyan.png'
import Yae_Miko from './img/Yae_Miko.png'
import Yanfei from './img/Yanfei.png'
import Yaoyao from './img/Yaoyao.png'
import Yelan from './img/Yelan.png'
import Yoimiya from './img/Yoimiya.png'
import Yun_Jin from './img/Yun_Jin.png'
import Zhongli from './img/Zhongli.png'


export const characterList = [
    { characterName: "Albedo", url: Albedo },
    { characterName: "Alhaitham", url: Alhaitham },
    { characterName: "Aloy", url: Aloy },
    { characterName: "Amber", url: Amber },
    { characterName: "Arataki Itto", url: Arataki_Itto },
    { characterName: "Arlecchino", url: Arlecchino },
    { characterName: "Baizhu", url: Baizhu },
    { characterName: "Barbara", url: Barbara },
    { characterName: "Beidou ", url: Beidou },
    { characterName: "Bennet ", url: Bennet },
    { characterName: "Candace ", url: Candace },
    { characterName: "Charlotte ", url: Charlotte },
    { characterName: "Chevreuse ", url: Chevreuse },
    { characterName: "Chiori", url: Chiori },
    { characterName: "Chongyun ", url: Chongyun },
    { characterName: "Clorinde", url: Clorinde },
    { characterName: "Collei", url: Collei },
    { characterName: "Cyno", url: Cyno },
    { characterName: "Dehya", url: Dehya },
    { characterName: "Diluc", url: Diluc },
    { characterName: "Diona", url: Diona },
    { characterName: "Dori", url: Dori },
    { characterName: "Eula", url: Eula },
    { characterName: "Faruzan", url: Faruzan },
    { characterName: "Fischl", url: Fischl },
    { characterName: "Freminet", url: Freminet },
    { characterName: "Furina ", url: Furina },
    { characterName: "Gaming", url: Gaming },
    { characterName: "Ganyu", url: Ganyu },
    { characterName: "Gorou", url: Gorou },
    { characterName: "Hu Tao", url: Hu_Tao },
    { characterName: "Jean", url: Jean },
    { characterName: "Kaedehara Kazuha", url: Kaedehara_Kazuha },
    { characterName: "Kaeya", url: Kaeya },
    { characterName: "Kamisato Ayaka", url: Kamisato_Ayaka },
    { characterName: "Kamisato Ayato", url: Kamisato_Ayato },
    { characterName: "Kaveh", url: Kaveh },
    { characterName: "Keqing", url: Keqing },
    { characterName: "Kirara", url: Kirara },
    { characterName: "Klee", url: Klee },
    { characterName: "Kujou Sara", url: Kujou_Sara },
    { characterName: "Kuki Shinobu", url: Kuki_Shinobu },
    { characterName: "Layla", url: Layla },
    { characterName: "Lisa", url: Lisa },
    { characterName: "Lynette", url: Lynette },
    { characterName: "Lyney", url: Lyney },
    { characterName: "Mika", url: Mika },
    { characterName: "Mona", url: Mona },
    { characterName: "Nahida", url: Nahida },
    { characterName: "Navia ", url: Navia },
    { characterName: "Neuvillette", url: Neuvillette },
    { characterName: "Nilou", url: Nilou },
    { characterName: "Ningguang", url: Ningguang },
    { characterName: "Noelle", url: Noelle },
    { characterName: "Qiqi", url: Qiqi },
    { characterName: "Raiden Shogun", url: Raiden_Shogun },
    { characterName: "Razor", url: Razor },
    { characterName: "Rosaria", url: Rosaria },
    { characterName: "Sangonomiya Kokomi", url: Sangonomiya_Kokomi },
    { characterName: "Sayu", url: Sayu },
    { characterName: "Sethos", url: Sethos },
    { characterName: "Shenhe", url: Shenhe },
    { characterName: "Shikanoin Heizou", url: Shikanoin_Heizou },
    { characterName: "Sigewinne", url: Sigewinne },
    { characterName: "Sucrose", url: Sucrose },
    { characterName: "Tartaglia", url: Tartaglia },
    { characterName: "Thoma", url: Thoma },
    { characterName: "Tighnari", url: Tighnari },
    { characterName: "Traveler(Anemo)", url: traveler_Anemo },
    { characterName: "Traveler(Dendro)", url: traveler_Dendro },
    { characterName: "Traveler(Electro)", url: traveler_Electro },
    { characterName: "Traveler(Geo)", url: traveler_Geo },
    { characterName: "Traveler(Hydro)", url: traveler_Hydro },
    { characterName: "Venti", url: Venti },
    { characterName: "Wanderer", url: Wanderer },
    { characterName: "Wriothesley ", url: Wriothesley },
    { characterName: "Xiangling", url: Xiangling },
    { characterName: "Xianyun", url: Xianyun },
    { characterName: "Xiao", url: Xiao },
    { characterName: "Xingqiu", url: Xingqiu },
    { characterName: "Xinyan", url: Xinyan },
    { characterName: "Yae Miko", url: Yae_Miko },
    { characterName: "Yanfei", url: Yanfei },
    { characterName: "Yaoyao", url: Yaoyao },
    { characterName: "Yelan", url: Yelan },
    { characterName: "Yoimiya", url: Yoimiya },
    { characterName: "Yun Jin", url: Yun_Jin },
    { characterName: "Zhongli", url: Zhongli },
]


// eslint-disable-next-line import/no-anonymous-default-export
export default {
    characterList,
}
